...
from clientUDP_listener import ClientUDP_Listener
import time
...
client = ClientUDP_Listener('127.0.0.1',52733)
client.start()

client2 = ClientUDP_Listener('127.0.0.1',52734)
client2.start()

client3 = ClientUDP_Listener('127.0.0.1',52735)
client3.start()
...
with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity = 2, static_image_mode = False,enable_segmentation = True) as pose: 
    ...
    while cap.isOpened():
        ...
        if results.pose_world_landmarks:
            ...
            client3.sendMessage(os.getppid())


            returnval=client2.readMessage();
            if returnval=="End":
                break
                #outward=outward
            #print(client2.readMessage())
